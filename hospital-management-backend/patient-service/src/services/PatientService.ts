...
export class PatientService {
  ...
  async createPatient(patientData: Partial<Patient>): Promise<PatientResult> {
    try {
      ...
      // Allow frontend to supply a specific patient ID (e.g., to match Auth user ID)
      const suppliedId = (patientData as any).id as string | undefined;
      const patientId = suppliedId && typeof suppliedId === 'string' && suppliedId.length > 0
        ? suppliedId
        : uuidv4();
